...
2. Существует окно $R_0$ больше единицы (в нашей модели, это приблизительно от $1$ до $1.5$ ), когда удерживание коэффициента в этом диапазоне приводит к бесконечной эпидемии. Это почти не зависит от того, сколько людей вакцинировано. Если коэффициент $R_0$ в этом диапазоне, то мы имеем бесконечную эпидемию.
3. Выход $R_0$ за пределы этого окна вверх приводит к тому, что эпидемия успевает закончиться за время иммунитета, но при этом число переболевших является значительным. В рамках нашей модели, без вакцинации это приводит к циклическому появлению новых штаммов, способных обходить имунные защиты предыдущих штаммов и, как результат, мы опять получаем бесконечность.
4. Если высокий коэффициент $R_0$ сочетается с высокой степенью вакцинации ($> 90\%$ в нашей модели ), что соответствует очень высокой заразности вируса (без вакцинации коэффициент распространения должен быть больше 10), то это приводит и к конечности эпидемии и к невысокому количеству переболевших, а значит и к низкой вероятности возникновения нового опасного штамма. Удивительно, но выглядит так, что этот вариант так же хорош как и $R_0$ незначительно меньше 1. Так что в этой ситуации, локдауны и ограничительные меры могут принести даже вред, потому что могут загнать $R_0$ в окно бесконечности. 

Конечно, нужно понимать, что это лишь примитивная математическая модель, а так же вспомнить и о других факторах, таких как смертность и перегруженность системы здравоохранения, которые никак не учитываются в нашем рассмотрении. Поэтому я не призываю ни к каким поспешным выводам, и главный вывод, который я бы сделал, так это то, что если нет возможности загнать $R_0$ к нулю, то все становится не так уж и очевидно.

И еще один небольшой неожиданный (во всяком случае для меня) результат.

Давайте возьмем какой-то $R_0$ из окна "бесконечности", например $R_0 = 1.3$ и посмотрим повнимательнее, как будет выглядеть $n[i]$. Точнее нас будет интересовать $n[i]/n[i-1]$.

А выглядеть он будет вот так (график приведен для 10 лет):



![image-20211205000445425](/Users/Roman/Library/Application Support/typora-user-images/image-20211205000445425.png)



Мы видим колебания с периодом $2 imm\_time$  (так если время иммунитета 1 год, то это 2 года).

И мы видим, что каждые 2 года кажется, что инфекция побеждена, что коэффициент падает, но в действительности это лишь синусоида. Это не вычислительная ошибка. Это связано с тем, что люди через год теряют иммунитет и пополняют собой число тех, кто может быть инфицирован. И при периоде в 2 года мы получаем смещение фазы на $\pi$. То есть, если коэффициент находится в минимуме, то это значит, что прирост тех, кто может быть инфицирован за счет потери иммунитета будет находиться на максимуме и это дает увеличение коэффициента, и наоборот.

Таким образом, когда $R_0$ находится в "окне бесконечности"  то каждые 2 года (при длительности иммунитета в 1 год) мы имеют картину ложного победы над вирусом.

![image-20211205190351531](/Users/Roman/Library/Application Support/typora-user-images/image-20211205190351531.png)
